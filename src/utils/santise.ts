...
export function sanitizeUserInput(input: string, opts: SanitizeOptions = {}): SanitizeResult {
  const { maxLength = 5_000, maxUrlsBeforeExcessive = 3, blacklist = DEFAULT_BLACKLIST } = opts;
  ...
  const JSY_RE = /(javascript:|data:text\/html|on\w+\s*=|<\s*script|\bdocument\.|\bwindow\.)/i;
  ...
  // Spam heuristics
  const URL_RE = /\b((https?:\/\/|ftp:\/\/|www\.)[^\s<>"')]+|[a-z0-9.-]+\.[a-z]{2,}\/[^\s<>"')]*)/gi;
  const urls: string[] = [];
  let match: RegExpExecArray | null;
  while ((match = URL_RE.exec(NEUTRALIZED))) {
    urls.push(match[0]);
  }
  ...
  const blacklistedWord = blacklist.some((phrase) => lower.includes(phrase.toLowerCase()));

  // Enforce max length
  const lengthExceeded = NEUTRALIZED.length > maxLength;
  const truncated = lengthExceeded ? NEUTRALIZED.slice(0, maxLength) : NEUTRALIZED;
