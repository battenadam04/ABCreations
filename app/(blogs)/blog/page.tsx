...
import { findContent } from '~/utils/content';
...
export default async function Home({}) {
  const posts = await findContent(contentDir.BLOG_DIR);
          ...
          <div key={slug} className="flex flex-col overflow-hidden rounded-xl border border-gray-200 shadow-lg">
            <Link href={`/${slug}`}>
              <Image
                ...
                height={340}
                alt={title}
                src={`${blogContent.images.find((content) => content.value === image)?.image.src}`}
              />
